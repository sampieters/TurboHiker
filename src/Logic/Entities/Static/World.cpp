...
{
        this->boundaries = {
            static_cast<float>(CONST::WORLD::UPPER_BOUNDARY), static_cast<float>(CONST::WORLD::RIGHT_BOUNDARY),
...
void turbohiker::World::InitWorld()
{
        ...
        for (int i = 0; i < CONST::ENEMIES::COMPETING::NUMBER; ++i) {
                ...
                comp->setPosition(
                    {std::dynamic_pointer_cast<turbohiker::Lane>(entities[CONST::ENEMIES::COMPETING::LANE[i]])
                         ->getPosition()
                         .first,
                     -2.5});
...
bool turbohiker::World::HorizontalOutOfBoundaries(float y) { return y < boundaries[2] || y > boundaries[0]; }

const std::vector<std::shared_ptr<turbohiker::Entity>>& turbohiker::World::getEntities() const { return entities; }
        ...
        for (const auto& entity : entities) {
                ...
                if (enemy != nullptr) {
                        if (player->getPosition().first == enemy->getPosition().first) {
                                if (enemy->getPosition().second > player_pos.second &&
                                    enemy->getPosition().second < enemy_pos_y) {
...
bool turbohiker::World::IsCollision(const std::shared_ptr<DynamicEntity>& to_check)
{
        for (const auto& entity_2 : entities) {
                ...
                if (to_check == entity_2 || std::dynamic_pointer_cast<turbohiker::Static>(entity_2) != nullptr &&
                                                std::dynamic_pointer_cast<turbohiker::Attacking>(to_check) != nullptr) {
                        continue;
                }
                ...
                if (dynamic_2 != nullptr) {
                        if (dynamic_2->getLane() == to_check->getLane()) {
                                if ((to_check->GetFront().second <= dynamic_2->GetFront().second &&
                                     to_check->GetFront().second >= dynamic_2->GetBack().second) ||
                                    (to_check->GetBack().second <= dynamic_2->GetFront().second &&
                                     to_check->GetBack().second >= dynamic_2->GetBack().second)) {
                                        return true;
                                }
                        }
                }
...
void turbohiker::World::CheckWhereToMove(const std::shared_ptr<Enemy>& enemy)
{
        ...
        if (GetClosestEnemy(enemy) != nullptr) {
                ...
                if (enemy->getLane()->getNext() != nullptr) {
                        ...
                        enemy->MoveRight();
                        if (IsCollision(enemy)) {
                                // Collision on the right. Now check if you could move left, if not preference is
                                // Cannot.
                                if (pref != Left) {
                                        pref = Cannot;
                                }
...
const std::shared_ptr<turbohiker::Score>& turbohiker::World::getScore() const { return score; }

void turbohiker::World::setScore(const std::shared_ptr<Score>& score) { World::score = score; }
