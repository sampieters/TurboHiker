...
turbohikerSFML::Camera::Camera(std::shared_ptr<turbohiker::Score> score, std::shared_ptr<turbohiker::Player> player,
                               const std::shared_ptr<sf::RenderWindow>& window)
    : turbohikerSFML::Entity("camera.png"), turbohiker::Camera(std::move(score), std::move(player))
{
...
void turbohikerSFML::Camera::display()
{
        ...
        score_1.setPosition(new_sprite.getPosition().x - (new_sprite.getGlobalBounds().width / 2) + 30,
                            new_sprite.getGlobalBounds().top + 5);
