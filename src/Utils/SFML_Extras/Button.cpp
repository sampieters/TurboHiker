...
// 1.2) ButtonColor
void Button::setButtonColor(sf::Color color) { button.setFillColor(color); }

sf::Color Button::getButtonColor() { return button.getFillColor(); }
...
const sf::Texture* Button::getTexture() { return button.getTexture(); }
...
sf::Vector2f Button::getPosition() { return button.getPosition(); }

// 1.4) OutlineColor
void Button::setOutlineColor(sf::Color color) { button.setOutlineColor(color); }

sf::Color Button::getOutlineColor() { return button.getOutlineColor(); }

void Button::setOutlineSize(float size) { button.setOutlineThickness(size); }

float Button::getOutlineSize() { return button.getOutlineThickness(); }
...
void Button::setFontSize(unsigned int size) { text.setCharacterSize(size); }

sf::Vector2f Button::getSize() { return button.getSize(); }

void Button::setSize(sf::Vector2f size) { button.setSize(size); }
...
void Button::setOrigin(sf::Vector2f point) { button.setOrigin(point); }

sf::Vector2f Button::getOrigin() { return this->button.getOrigin(); }

void Button::setClickSound(sf::SoundBuffer buffer) { click = buffer; }

bool Button::isMouseOver(sf::Vector2i mouse)
{
        return (
            mouse.x > (getPosition().x - getOrigin().x) && mouse.x < (getPosition().x + getSize().x - getOrigin().x) &&

            mouse.y > (getPosition().y - getOrigin().y) && mouse.y < (getPosition().y + getSize().y - getOrigin().y));
}
...
void Button::setText(const std::string& text) { Button::text.setString(text); }

void Background::draw(sf::RenderTarget& target, sf::RenderStates states) const { target.draw(spirte); }
