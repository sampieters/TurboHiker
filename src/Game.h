...
public:
    ...
    void TransitionTo(const std::shared_ptr<T_State>& state) {
        ...
        if(this->state_ == nullptr || this->window == nullptr) {
            throw exceptions::NullptrException("Game state is not initialized correctly, leading to a nullptr");
        }
    }
     ...
     * Getters and Setters
     */
    const std::shared_ptr<turbohiker::World> &getWorld() const {
        return world_;
    }

    void setWorld(std::shared_ptr<turbohiker::World> world) {
