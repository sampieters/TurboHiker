...
const std::shared_ptr<sf::Font> &T_State::getGuiFont() const {
    return gui_font;
}

void T_State::setGuiFont(const std::shared_ptr<sf::Font> &guiFont) {
    gui_font = guiFont;
}

const std::shared_ptr<sf::Music> &T_State::getGuiSound() const {
    return gui_sound;
}

void T_State::setGuiSound(const std::shared_ptr<sf::Music> &guiSound) {
    gui_sound = guiSound;
}

const std::vector<std::shared_ptr<sf::Drawable>> &T_State::getGuiObjects() const {
    return gui_objects;
}

void T_State::setGuiObjects(const std::vector<std::shared_ptr<sf::Drawable>> &guiObjects) {
    gui_objects = guiObjects;
}

void T_State::set_context(std::shared_ptr<T_Game> context) {
...
T_Game::T_Game(const std::shared_ptr<T_State>& state): state_(state) {
    try {
        window = std::make_shared<sf::RenderWindow>(sf::VideoMode(CONST::WINDOW::WIDTH, CONST::WINDOW::HEIGHT), "TurboHiker");
...
const std::string &T_Game::getPlayerName() const {
    return player_name;
}

void T_Game::setPlayerName(const std::string &playerName) {
    player_name = playerName;
}

bool T_Game::isRunning() const {
    return running;
}

void T_Game::setRunning(bool running) {
    T_Game::running = running;
}

void T_Game::setPreviousState(const std::shared_ptr<T_State> &previousState) {
    previous_state_ = previousState;
}

const std::shared_ptr<T_State> &T_Game::getPreviousState() const {
    return previous_state_;
}

void T_Game_Menu::init() {
    if (this->context_ == nullptr) {
        throw exceptions::NullptrException("Game state or Window is not initialized correctly, leading to a nullptr");
    }
    ...
    sf::Vector2f button_size = {static_cast<float>(this->context_->getWindow()->getSize().x/4), static_cast<float>(this->context_->getWindow()->getSize().y/8)};
    ...
    while (this->context_->getWindow()->pollEvent(event)) {
        switch (event.type) {
            ...
            case sf::Event::Resized:
                this->context_->getWindow()->setView(sf::View(sf::FloatRect(0, 0, event.size.width, event.size.height)));
                break;
                                    ...
                                    for(const auto& object: this->getGuiObjects()) {
                                        if (std::dynamic_pointer_cast<Textbox>(object) != nullptr) {
                                            ...
                                            if(!textbox->getText().empty()) {
                                                this->context_->setPlayerName(textbox->getText());
                                                this->context_->TransitionTo(std::make_shared<T_Game_Play>());
...
void T_Game_Play::init() {
    ...
    if(!this->context_->isRunning()) {
        ...
        this->context_->setWorld(std::dynamic_pointer_cast<turbohiker::World>(wow->CreateWorld(this->context_->getPlayerName())));
...
void T_Game_Play::run() {
    ...
    while (this->context_->getWindow()->isOpen()) {
        ...
        if(this->context_->getWorld()->isStop()) {
            this->context_->setWindow(std::make_shared<sf::RenderWindow>(sf::VideoMode(CONST::WINDOW::WIDTH, CONST::WINDOW::HEIGHT), "TurboHiker"));
...
void T_Game_Scoreboard::init() {
    ...
    score->setPosition((this->context_->getWindow()->getSize().x/2), (this->context_->getWindow()->getSize().y/2));
